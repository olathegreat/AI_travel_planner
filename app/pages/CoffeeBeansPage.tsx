  ...
  return (
    <View className="flex-1 bg-primary  pt-10">
      ...
      <View className="bg-black/50 -mt-40 rounded-t-3xl p-5 min-h-40 flex flex-row justify-between">
        ...
        delay={500}
        duration={1000}
        useNativeDriver className="flex justify-between">
          <View className="flex flex-row gap-2">
            ...
            <View className="bg-gray-900 justify-center h-16 w-16 rounded-lg flex items-center gap-1">
              ...
              <Text className="text-white font-outfit">
                {parsedData.shortDesc.split(" ")[parsedData.shortDesc.split(" ").length -1]}
              </Text>
            </View>
          </View>
      ...
      <View className="p-5">
        <Text className="text-white font-outfit text-xl mb-4">Description</Text>
        ...
        <View className="flex flex-row gap-4">
          ...
          {  parsedData?.sizes?.map((item, index) => (
            <TouchableOpacity key={index} onPress={()=>{
              ...
              <Text className={` ${selectedSize === item.size ? "text-secondary" : "text-white"}`}>{item.size}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
      ...
      <Animatable.View animation="fadeInUp"
        delay={500}
        duration={1000}
        useNativeDriver className="p-5 mt-4 flex flex-row justify-between items-center w-full">

        <View className="flex items-center ">
          <Text className="text-white font-outfit">Price</Text>
          <View className="flex flex-row gap-2">
          <Text className="text-secondary font-outfit-medium text-3xl">$</Text>
          <Text className="text-white font-outfit-medium text-3xl">{" "}
                {parsedData.sizes?.find(
                  (item) => item.size === selectedSize
                )?.price.toFixed(2) || parsedData.price}</Text>
          </View>
        ...
        className="bg-secondary p-4 rounded-xl flex justify-center items-center w-[200px] ">
          <Text className="text-white text-outfit-medium text-2xl text-center">Add to Cart</Text>
          </TouchableOpacity>
